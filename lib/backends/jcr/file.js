...
JCRFile.createInstance = function (filePath, tree, content, fileLength, cb) {
  ...
  function getContent(callback) {
    if (content) {
      ...
      tree.getContent(filePath, false, function (err, content) {
        if (content) {
...
JCRFile.prototype.applyRequestDefaults = function(options) {
  return this.tree.applyRequestDefaults(options);
};

JCRFile.prototype.invalidateContentCache = function (path, deep) {
  this.tree.invalidateContentCache(path, deep);
};

JCRFile.prototype.getContent = function (path, deep) {
  this.tree.getContent(path, deep);
};

JCRFile.prototype.getLocalFile = function (path, lastModified, cb) {
  this.tree.getLocalFile(path, lastModified, cb);
};
...
JCRFile.prototype.ensureGotLocalCopy = function (cb) {
  ...
  var self = this;
  this.getLocalFile(this.filePath, this.lastModified(), function (err, localFilePath) {
    if (err) {
      cb(err);
    } else {
        ...
        if (err) {
          ...
          fs.fstat(localFile.fd, function (err, stats) {
            if (err) {
              ...
              self.invalidateContentCache(self.filePath, false);
  ...
  this.ensureGotLocalCopy(function (err, localFile) {
    ...
    fs.ftruncate(localFile.fd, length, function (err) {
      if (err) {
        ...
        self.getContent(self.filePath, false, function (err, content) {
          if (content) {
...
JCRFile.prototype.delete = function (cb) {
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      // succeeded
      // invalidate cache
      self.invalidateContentCache(self.filePath, self.isDirectory());
      self.dirty = false;
      if (self.localFile) {
