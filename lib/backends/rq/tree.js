...
RQTree.prototype.handleErr = function (context, cb, err) {
 ...
 */
RQTree.prototype.isQueuedFor = function (context, name, methods, cb) {
  var self = this;
  self.rq.getRequests(context, utils.getParentPath(name), function (err, lookup) {
    if (err) {
      self.handleErr(context, cb, err);
    } else {
      var i;
 ...
 */
RQTree.prototype.createFileInstanceFromOpen = function (context, openFile, cb) {
  RQFile.createInstance(context, openFile, this, cb);
};
 ...
 */
RQTree.prototype.existsExt = function (context, name, cb) {
  context.rq().debug('RQTree.exists %s', name);
  context.spi().debug('[%s] tree.exists %s', this.share.config.backend, name);
  // first check to see if the file exists locally
  var self = this;
  this.local.existsExt(context, name, function (err, result) {
    if (err) {
      self.handleErr(context, cb, err);
    }  else {
      if (result) {
        ...
        self.rq.getRequests(context, utils.getParentPath('/'), function (err, lookup) {
          if (err) {
            self.handleErr(context, cb, err);
          } else {
            self.isQueuedFor(context, name, ['DELETE'], function (err, deleted) {
              if (err) {
                self.handleErr(context, cb, err);
              } else if (!deleted) {
                // check to see if the file exists remotely
                context.rq().debug('RQTree.exists.remote.exists %s', name);
                self.remote.existsExt(context, self.remoteEncodePath(name), function (err, result) {
                  if (err) {
                    self.handleErr(context, cb, err);
                  } else {
                    cb(null, result);
...
function _existsLocally(context, name, cb) {
  var self = this;
  self.local.existsExt(context, name, function (err, localExists) {
    if (err) {
      cb(err);
    } else {
      if (localExists) {
        cb(null, !self.local.isDownloading(context, name));
      } else {
        cb(null, false);
      }
    }
 ...
 */
RQTree.prototype.openExt = function (context, name, cb) {
  var logger = context.spi();
  var rqlog = context.rq();
  rqlog.debug('RQTree.open %s', name);
  logger.debug('[%s] tree.open %s', this.share.config.backend, name);
  var self = this;
  _existsLocally.call(this, context, name, function (err, localExists) {
    if (err) {
      self.handleErr(context, cb, err);
    } else {
      if (localExists) {
        // local file exists
        self.local.openExt(context, name, function (err, localFile) {
          if (err) {
            self.handleErr(context, cb, err);
          } else {
            self.createFileInstanceFromOpen(context, localFile, cb);
          }
        });
      } else if (!self.isTempFileName(name)) {
        // local file does not exist
        rqlog.debug('RQTree.open.remote.open %s', name);
        self.remote.openExt(context, self.remoteEncodePath(name), function (err, remoteFile) {
          if (err) {
            self.handleErr(context, cb, err);
          } else {
            self.createFileInstanceFromOpen(context, remoteFile, cb);
          }
        });
      } else {
 ...
 * @private
 */
function _prepListData(context, tree, pattern, cb) {
  var logger = context.spi();
  ...
  tree.rq.getRequests(context, tree.remoteEncodePath(data.parentPath), function (err, requests) {
    if (err) {
      cb(err);
    } else {
      tree.local.existsExt(context, data.parentPath, function (err, exists) {
        if (err) {
          cb(err);
        } else {
...
function _convertRemoteToRqFile(data, cb) {
  ...
  tree.remote.listExt(context, pattern, function (err, remoteFiles) {
    if (err) {
      cb(err);
    } else {
      async.each(remoteFiles, function (remoteFile, eachCb) {
        if (tree.isTempFileName(remoteFile.getPath())) {
          ...
          if (existingRequests[tree.remoteEncodePath(remoteFile.getName())] != 'DELETE') {
            tree.createFileInstanceFromOpen(context, remoteFile, function (err, newFile) {
              if (err) {
                eachCb(err);
              } else {
 ...
 */
function _deleteLocalFile(context, tree, localFile, cb) {
  var logger = context.spi();
  ...
  localFile.canDelete(function (err, canDelete) {
    if (err) {
      ...
      if (localFile.isDirectory()) {
        ...
        tree.deleteLocalDirectoryRecursive(context, localFile.getPath(), function (err) {
          if (err) {
            cb(err);
          } else {
            cb();
        ...
        tree.local.deleteExt(context, localFile.getPath(), cb);
      }
    } else {
...
function _mergeLocalFiles(data, cb) {
  ...
  if (data.localExists) {
    tree.local.listExt(context, data.pattern, function (err, localFiles) {
      if (err) {
        cb(err);
      } else {
        async.each(localFiles, function (localFile, eachCb) {
          if (_ignoreList(localFile.getName())) {
            ...
            tree.createFileInstanceFromOpen(context, localFile, function (err, rqFile) {
              if (err) {
                eachCb(err);
              } else {
            ...
            if (remoteIndex !== undefined) {
              ...
              tree.createFileInstanceFromOpen(context, localFile, function (err, rqFile) {
                if (err) {
                  ...
                  tree.isCreatedLocally(context, localFile.getPath(), function (err, exists) {
                    if (err) {
                      eachCb(err);
                    } else {
                      if (exists) {
                        ...
                        _deleteLocalFile(context, tree, localFile, function (err, isConflict) {
                          if (err) {
                            eachCb(err);
                          } else {
                            if (isConflict) {
 ...
 */
RQTree.prototype.listExt = function (context, pattern, cb) {
  var logger = context.spi();
  var rqlog = context.rq();
  ...
  if (filter == '*') {
    ...
    self.share.getListCache(context, dirPath, self, function (err, list) {
      if (err) {
        self.handleErr(context, cb, err);
      } else if (list) {
        ...
        async.waterfall([
          async.apply(_prepListData, context, self, pattern),
    ...
    var processRq = function (err, files) {
      if (err) {
        ...
        self.createFileInstanceFromOpen(context, targetFile, function (err, rqFile) {
          if (err) {
            self.handleErr(context, cb, err);
          } else {
            if (files.length) {
    ...
    self.local.existsExt(context, pattern, function (err, exists) {
      if (err) {
        self.handleErr(context, cb, err);
      } else if (exists) {
        // local item exists, use local result
        self.local.listExt(context, pattern, function (err, files) {
          processRq(err, files);
        });
      } else if (!self.isTempFileName(pattern)) {
        self.isQueuedFor(context, pattern, ['DELETE'], function (err, deleted) {
          if (err) {
            self.handleErr(context, cb, err);
          } else if (!deleted) {
            // use remote result
            self.remote.listExt(context, pattern, function (err, files) {
              processRq(err, files);
            });
          } else {
            // does not exist
 ...
 */
RQTree.prototype.refreshWorkFiles = function (context, filePath, cb) {
  var logger = context.spi();
  var rqlog = context.rq();
  rqlog.debug('RQTree.refreshWorkFiles %s', filePath);
  var self = this;
  logger.info('refreshing work files for %s', filePath);
  self.remote.openExt(context, self.remoteEncodePath(filePath), function (err, remote) {
    if (err) {
      self.handleErr(context, cb, err);
    } else {
      self.local.refreshCacheInfo(context, filePath, remote, function (err) {
        if (err) {
          self.handleErr(context, cb, err);
        } else {
          cb();
 ...
 */
RQTree.prototype.canDelete = function (context, path, cb) {
  var logger = context.spi();
  var rqlog = context.rq();
  var self = this;
  ...
  self.local.existsExt(context, path, function (err, exists) {
    if (err) {
      self.handleErr(context, cb, err);
    } else if (exists) {
      self.local.openExt(context, path, function (err, localFile) {
        if (err) {
          self.handleErr(context, cb, err);
        } else {
          localFile.canDelete(function (err, canDelete) {
...
function _processDirectory(context, tree, toProcess, cb) {
  var logger = context.spi();
  var processDirs = [{
    name: toProcess
  }];
  ...
  async.whilst(function () {
    ...
    tree.local.listExt(context, Path.join(dirName, '/*'), function (err, items) {
      if (err) {
        whilstCb(err);
      } else {
        ...
        async.eachSeries(items, function (item, eachCb) {
          if (item.isDirectory()) {
            ...
            _deleteFile(context, tree, item, function (err, wasDeleted) {
              if (err) {
                eachCb(err);
              } else {
                if (!wasDeleted) {
    ...
    if (err) {
      ...
      _deleteDirs(context, tree, processDirs, skipDelete, cb);
    }
  });
}

function _deleteDirs(context, tree, processDirs, skipDelete, cb) {
  var logger = context.spi();
  ...
  async.eachSeries(processDirs, function (dir, eachCb) {
    ...
    if (skipDelete[dirName]) {
      ...
      if (dirName != '/') {
        ...
        tree.local.deleteDirectoryExt(context, dirName, eachCb);
      } else {
...
function _deleteFile(context, tree, toDelete, cb) {
  var logger = context.spi();
  ...
  toDelete.canDelete(function (err, canDelete) {
    if (err) {
      ...
      logger.debug('deleting file %s', toDelete.getPath());
      tree.local.deleteExt(context, toDelete.getPath(), function (err) {
        if (err) {
          cb(err);
        } else {
          cb(null, true);
 ...
 */
RQTree.prototype.deleteLocalDirectoryRecursive = function (context, name, cb) {
  var rqlog = context.rq();
  var logger = context.spi();
  ...
  _processDirectory(context, self, name, function (err) {
    if (err) {
      self.handleErr(context, cb, err);
    } else {
      cb();
 ...
 */
RQTree.prototype.queueDataExists = function (context, name, cb) {
  var self = this;
  var encoded = self.remoteEncodePath(name);
  self.rq.exists(context, utils.getParentPath(encoded), utils.getPathName(encoded), cb);
};
 ...
 */
RQTree.prototype.queueData = function (context, name, method, newName, moreOpts, cb) {
  var logger = context.spi();
  var rqlog = context.rq();
  ...
  if (!isTempFile) {
    logger.info('queuing %s for %s', method, name);
    this.rq.queueRequest(context, options, function (err) {
      if (err) {
 ...
 */
RQTree.prototype.isCreatedLocally = function (context, name, cb) {
  var rqlog = context.rq();
  rqlog.debug('RQTree.isCreatedLocally %s', name);
  var self = this;
  self.local.isCreatedLocally(context, name, function (err, isCreatedLocally) {
    if (err) {
      self.handleErr(context, cb, err);
    } else {
 ...
 */
RQTree.prototype.createFileExt = function (context, name, cb) {
  var rqlog = context.rq();
  var logger = context.spi();
  ...
  var self = this;
  self.local.createFileExt(context, name, function (err, file) {
    if (err) {
      self.handleErr(context, cb, err);
    } else {
      self.share.invalidateContentCache(context, utils.getParentPath(name), true);
      self.createFileInstanceFromOpen(context, file, cb);
    }
  });
};
 ...
 */
RQTree.prototype.createDirectoryExt = function (context, name, cb) {
  var rqlog = context.rq();
  var logger = context.spi();
  ...
  var self = this;
  self.local.createDirectoryExt(context, name, function (err, file) {
    if (err) {
      self.handleErr(context, cb, err);
    } else {
      ...
      if (!self.isTempFileName(name)) {
        ...
        self.remote.createDirectoryExt(context, self.remoteEncodePath(name), function (err, remoteDir) {
          if (err) {
            self.handleErr(context, cb, err);
          } else {
            self.createFileInstanceFromOpen(context, file, cb);
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQTree.prototype.deleteExt = function (context, name, cb) {
  var rqlog = context.rq();
  var logger = context.spi();
  rqlog.debug('RQTree.delete %s', name);
  logger.debug('[%s] tree.delete %s', this.share.config.backend, name);
  var self = this;
  self.local.getPathInfo(context, name, function (err, exists) {
    if (err) {
      self.handleErr(context, cb, err);
    } else {
      self.share.invalidateContentCache(context, utils.getParentPath(name), true);
      if (exists) {
        self.isCreatedLocally(context, name, function (err, createExists) {
          if (err) {
            self.handleErr(context, cb, err);
          } else {
            logger.info('deleting %s from cache', name);
            self.local.deleteExt(context, name, function (err) {
              if (err) {
                self.handleErr(context, cb, err);
              } else {
                self.queueDataExists(context, name, function (err, queueExists) {
                  if (err) {
                    self.handleErr(context, cb, err);
                  } else {
                    if (!createExists || queueExists) {
                      self.queueData(context, name, 'DELETE', false, function (err) {
                        if (err) {
        ...
        self.handleErr(context, cb, 'path ' + name + ' to delete does not exist');
      }
    }
  });
};
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQTree.prototype.deleteDirectoryExt = function (context, name, cb) {
  var rqlog = context.rq();
  var logger = context.spi();
  ...
  var deleteRemote = function (callback) {
    if (!self.isTempFileName(name)) {
      ...
      self.remote.deleteDirectoryExt(context, self.remoteEncodePath(name), callback);
    } else {
      callback();
    }
  };

  self.local.existsExt(context, name, function (err, exists) {
    if (err) {
      self.handleErr(context, cb, err);
    } else {
      self.share.invalidateContentCache(context, utils.getParentPath(name), true);
      if (exists) {
        ...
        self.local.deleteDirectoryExt(context, name, function (err) {
          if (err) {
            self.handleErr(context, cb, err);
          } else {
            deleteRemote(function (err) {
              if (err) {
                ...
                self.rq.removePath(context, self.remoteEncodePath(name), function (err) {
                  if (err) {
                    self.handleErr(context, cb, err);
                  } else {
                    cb();
 ...
 * @private
 */
function _getReplaceInfo(context, oldName, newName, cb) {
  var logger = context.spi();
  var self = this;
  var info = {};

  function _isDirectory(callback) {
    self.local.getPathInfo(context, oldName, function (err, exists, isDir) {
      if (err) {
        cb(err);
      } else if (!exists) {
        var remoteOldName = self.remoteEncodePath(oldName);
        self.remote.existsExt(context, remoteOldName, function (err, existsRemote) {
          if (err) {
            cb(err);
          } else if (existsRemote) {
            self.remote.openExt(context, remoteOldName, function (err, file) {
              if (err) {
                cb(err);
              } else {
  ...
  function _getNewInfo(callback) {
    if (info.isDir) {
      ...
      self.isQueuedFor(context, newName, ['DELETE', 'POST'], function (err, queued) {
        if (err) {
          callback(err);
        } else {
          self.local.getPathInfo(context, newName, function (err, newExistsLocal) {
            if (err) {
              callback(err);
            } else {
  ...
  function _getRemoteInfo(callback) {
    if (!self.isTempFileName(newName)) {
      ...
      self.remote.existsExt(context, remotePath, function (err, exists) {
        if (err) {
          callback(err);
        } else {
          info['newExists'] = exists;
          if (exists) {
            self.remote.openExt(context, remotePath, function (err, remoteFile) {
              if (err) {
                callback(err);
              } else {
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQTree.prototype.renameExt = function (context, oldName, newName, cb) {
  var rqlog = context.rq();
  var logger = context.spi();
  ...
  var renameRemoteDir = function (callback) {
    ...
    if (!self.isTempFileName(oldName) && !self.isTempFileName(newName)) {
      ...
      self.remote.renameExt(context, self.remoteEncodePath(oldName), self.remoteEncodePath(newName), callback);
    } else {
  ...
  _getReplaceInfo.call(this, context, oldName, newName, function (err, info) {
    if (err) {
      self.handleErr(context, cb, err);
    } else {
      if (info.oldExistsLocal) {
        logger.info('renaming %s to %s in local cache', oldName, newName);
        self.local.renameLocal(context, oldName, newName, info.newRemote, function (err) {
          if (err) {
            self.handleErr(context, cb, err);
          } else {
            logger.debug('%s successfully renamed to %s', oldName, newName);
            // invalidate cache
            self.share.invalidateContentCache(context, utils.getParentPath(oldName), true);
            self.share.invalidateContentCache(context, utils.getParentPath(newName), true);
            ...
            if (info.isDir) {
              ...
              renameRemoteDir(function (err) {
                if (err) {
                  ...
                  self.rq.updatePath(context, self.remoteEncodePath(oldName), self.remoteEncodePath(newName), function (err) {
                    if (err) {
                      self.handleErr(context, cb, err);
                    } else {
                      logger.debug('successfully updated queued requests for %s to %s', oldName, newName);
                      self.handleErr(context, cb);
                    }
                  });
                }
              });
              ...
              self.queueData(context, oldName, 'MOVE', newName, {replace: info.newExists}, function (err) {
                if (err) {
                  self.handleErr(context, cb, err);
                } else {
        ...
        if (info.oldExistsRemote) {
          ...
          self.remote.renameExt(context, self.remoteEncodePath(oldName), self.remoteEncodePath(newName), cb);
        } else {
          cb();
        }
      } else {
        logger.warn('attempting to rename temp file %s, which does not exist', oldName);
        self.handleErr(context, cb, 'cannot rename non-existing temp file ' + oldName);
      }
    }
  });
};
 ...
 */
RQTree.prototype.clearCacheExt = function (context, cb) {
  this.deleteLocalDirectoryRecursive(context, Path.sep, cb);
};
