 ...
 */
var RQTree = function (share, remote, options, context) {
  if (!(this instanceof RQTree)) {
    return new RQTree(share, remote, options, context);
  }
  options = options || {};
  this.options = options;
  this.remote = remote;
  this.local = new RQLocalTree(share, new FSTree(new FSShare('rqlocal', share.config.local), context), this, context);
  this.rq = share.rq;

  Tree.call(this, share.config, share, context);
};

// the RQTree prototype inherits from Tree
Util.inherits(RQTree, Tree);

RQTree.prototype.invalidateContentCache = function (path, deep) {
  this.share.invalidateContentCache(this.getContext(), path, deep);
};

RQTree.prototype.getRequestQueueLogger = function () {
  return this.getContext().rq();
};

RQTree.prototype.handleErr = function (cb, err) {
  var rqlog = this.getRequestQueueLogger();
  if (err) {
...
RQTree.prototype.isQueuedFor = function (name, methods, cb) {
  var self = this;
  self.rq.getRequests(self.getContext(), utils.getParentPath(name), function (err, lookup) {
    if (err) {
...
function _convertRemoteToRqFile(data, cb) {
  var tree = data.tree;
  var logger = tree.getLogger();
  var rqlog = tree.getRequestQueueLogger();
...
RQTree.prototype.list = function (pattern, cb) {
  ...
  if (filter == '*') {
    ...
    self.share.getListCache(self.getContext(), dirPath, self, function (err, list) {
      if (err) {
        self.handleErr(cb, err);
      } else if (list) {
...
RQTree.prototype.queueDataExists = function (name, cb) {
  ...
  self.rq.exists(this.getContext(), utils.getParentPath(encoded), utils.getPathName(encoded), cb);
};
...
RQTree.prototype.queueData = function (name, method, newName, moreOpts, cb) {
  ...
  if (!isTempFile) {
    ...
    this.rq.queueRequest(this.getContext(), options, function (err) {
      if (err) {
...
RQTree.prototype.createFile = function (name, cb) {
  ...
  self.local.createFile(name, function (err, file) {
    if (err) {
      ...
      self.invalidateContentCache(utils.getParentPath(name), true);
      ...
      if (info.oldExistsLocal) {
        ...
        logger.info('renaming %s to %s in local cache', oldName, newName);
          if (err) {
            ...
            // invalidate cache
            self.invalidateContentCache(utils.getParentPath(oldName), true);
            self.invalidateContentCache(utils.getParentPath(newName), true);
