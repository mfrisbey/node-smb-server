...
SMBTree.prototype.getLogCmdEndCallback = function (command, description, cb) {
  var smbcmd = this.getLogger();
  description = description || '';

  function _log() {
    if (debugCmds[command]) {
      smbcmd.debug.apply(smbcmd, arguments);
    } else {
      smbcmd.info.apply(smbcmd, arguments);
    }
  }

  _log('-> %s %s', command, description);
  return function (result) {
    ...
    _log('<- %s %s %s', status, command, description);
    cb(result);
  };
};

SMBTree.prototype.getContext = function () {
  return this.spiTree.getContext();
};

SMBTree.prototype.getLogger = function () {
  return this.spiTree.getLogger();
};
...
SMBTree.prototype.getFile = function (fid) {
  return this.getShare().getFile(fid);
};

SMBTree.prototype.setFile = function (fid, file) {
  this.getShare().setFile(fid, file);
};

SMBTree.prototype.closeFile = function (fid, cb) {
  this.getShare().closeFile(fid, cb);
};
...
SMBTree.prototype.registerChangeListener = function (mid, file, deep, completionFilter, cb) {
  this.getShare().registerChangeListener(mid, file, deep, completionFilter, cb);
};
...
SMBTree.prototype.notifyChangeListeners = function (action, name, newName) {
  this.getShare().notifyChangeListeners(action, name, newName);
};
...
SMBTree.prototype.cancelChangeListener = function (mid) {
  return this.getShare().cancelChangeListener(mid);
};
...
SMBTree.prototype.clearCache = function (cb) {
  ...
  if (self.spiTree) {
    ...
    self.getLogger().debug('cannot clear cache because spiTree is not an object');
    cb();
  }
};
