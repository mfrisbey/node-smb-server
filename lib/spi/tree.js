 ...
 * @this {Tree}
 */
var Tree = function (config, share, context) {
  if (!(this instanceof Tree)) {
    return new Tree(config, share, context);
  }
  this.config = config || {};
  this.share = share;
  this.context = context ? context : share.createContext().withLabel('SPITree.Constructor');
};

/**
 * Retrieves the unique request id associated with this tree.
 * @returns {string} The tree' request id.
 */
...
/**
 * Retrieves the tree's smb context.
 * @returns {SMBContext} Context information for the tree.
 */
...
/**
 * Retrieves the tree's logger object, which should be used to log messages.
