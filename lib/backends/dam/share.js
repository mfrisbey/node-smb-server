...
DAMShare.prototype.getContent = function (context, path, deep, cb) {
  var self = this;
  if (deep) {
    // for folder lists, use default implementation
    JCRShare.prototype.getContent.call(self, context, path, deep, cb);
  } else {
    ...
    self.getContent(context, parent, true, function (err, parentContent) {
      if (err) {
        cb(err);
      } else {
        if (parent == path) {
...
DAMShare.prototype.fetchContent = function (context, path, deep, cb) {
  if (path === Path.sep) {
    path = '';
  }
...
DAMShare.prototype.createTreeInstance = function (content, tempFilesTree, context) {
  return new DAMTree(this, content, tempFilesTree, context);
};

DAMShare.prototype.createTree = function (context) {
  return JCRShare.prototype.createTree.call(this, context);
};
