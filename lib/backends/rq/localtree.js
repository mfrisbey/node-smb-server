 ...
 * @private
 */
function _notifyDownloadComplete(context, path) {
  ...
  if (self.waits[path]) {
    ...
    for (i = 0; i < self.waits[path].length; i++) {
      ...
      var waitContext = self.waits[path][i].context;

      logger.info('%s download complete, notifying waiting thread %s', path, waitContext.getRequestId());
      waitContext.spi().info('%s download complete, wait callback invoked', path);
      waitCallback();
    }
    self.waits[path] = [];
  }
 ...
 * @private
 */
function _writeWorkData(context, workFile, cacheInfo, cb) {
  var logger = context.spi();
 ...
 * @private
 */
function _createWorkFile(context, targetFile, created, isRefreshed, remoteFile, cb) {
  var logger = context.spi();
  var name = targetFile.getPath();
  var self = this;

  function _openFile(toOpen) {
    self.source.openExt(context, toOpen, cb);
  }

  function _createFile(path) {
    self.source.createFileExt(context, path, function (err, workFile) {
      if (err) {
        cb(err);
      } else {
        logger.debug('%s work file created', path);
        _writeWorkData.call(self, context, workFile, RQLocalFile.getCacheInfo(targetFile, remoteFile, created, isRefreshed), function (err) {
          if (err) {
            cb(err);
          } else {
            _openFile(path);
  ...
  if (!self.isTempFileName(name) && !targetFile.isDirectory()) {
    ...
    self.source.existsExt(context, filePath, function (err, exists) {
      if (err) {
        cb(err);
      } else if (exists) {
        ...
        if (created) {
          ...
          self.source.deleteExt(context, filePath, function (err) {
            if (err) {
              cb(err);
            } else {
 ...
 */
RQLocalTree.prototype.getPathInfo = function (context, path, cb) {
  var self = this;

  function _exists(callback) {
    self.source.existsExt(context, path, function (err, exists) {
      if (err) {
        cb(err);
      } else {
        self.cacheInfoExists(context, path, function (err, cacheExists) {
          var checkDir = exists;
  ...
  _exists(function (exists, checkDir) {
    if (exists && checkDir) {
      ...
      self.source.openExt(context, path, function (err, item) {
        if (err) {
          cb(err);
        } else {
 ...
 *
 */
RQLocalTree.prototype.cacheInfoExists = function (context, path, cb) {
  if (!this.isTempFileName(path)) {
    // small optimization for temp files. only check for info file if it's not a temp file
    this.source.existsExt(context, this.getInfoFilePath(path), cb);
  } else {
    cb(null, false);
  }
};
 ...
 */
RQLocalTree.prototype.isCreatedLocally = function (context, path, cb) {
  var self = this;
  self.cacheInfoExists(context, path, function (err, exists) {
    if (err) {
      cb(err);
    } else if (!exists) {
      cb(null, false);
      ...
      self.source.openExt(context, infoFile, function (err, localFile) {
        if (err) {
          cb(err);
        } else {
 ...
 */
RQLocalTree.prototype.waitOnDownload = function (context, path, cb) {
  var logger = context.spi();
  var self = this;
  if (self.isDownloading(context, path)) {
    ...
    self.waits[path].push({ context: context, callback: cb });
  } else {
 ...
 */
RQLocalTree.prototype.isDownloading = function (context, path) {
  var logger = context.spi();
  if (!this.isTempFileName(path)) {
 ...
 */
RQLocalTree.prototype.setDownloading = function (context, path, isDownloading) {
  var wasDownloading = this.isDownloading(context, path);
  this.downloadingFiles[path] = isDownloading;
  if (wasDownloading && !isDownloading) {
    // the file is no longer downloading. remove lock
    _notifyDownloadComplete.call(this, context, path);
  }
};
 ...
 */
RQLocalTree.prototype.download = function (context, remote, name, cb) {
  var logger = context.spi();
  var self = this;

  function sendResult(err, file) {
    self.setDownloading(context, name, false);
    cb(err, file);
  }

  self.setDownloading(context, name, true);
  remote.openExt(context, self.rqTree.remoteEncodePath(name), function (err, remoteFile) {
    if (err) {
      sendResult(err);
    } else {
      logger.info('downloading file %s from remote', name);
      self.share.fetchResource(context, self.rqTree.remoteEncodePath(name), function (err, localPath) {
        if (err) {
          ...
          self.source.openExt(context, name, function (err, file) {
            if (err) {
              sendResult(err);
            } else {
 ...
 * @param {Error} cb.err Will be truthy if there were errors during the operation.
 */
RQLocalTree.prototype.refreshCacheInfo = function(context, name, remoteFile, cb) {
  var logger = context.spi();
  var self = this;
  var filePath = self.getInfoFilePath(name);

  function _deleteWork(delCb) {
    self.cacheInfoExists(context, name, function (err, exists) {
      if (err) {
        cb(err);
      } else if (exists) {
        self.source.deleteExt(context, filePath, function (err) {
  ...
  _deleteWork(function () {
    self.source.openExt(context, name, function (err, localFile) {
      if (err) {
        cb(err);
      } else {
        _createWorkFile.call(self, context, localFile, false, true, remoteFile, function (err, workFile) {
...
RQLocalTree.prototype.createFromSource = function (source, remote, isCreated, cb) {
  var self = this;
  _createWorkFile.call(self, source.context, source, isCreated, false, remote, function (err, workFile) {
    if (err) {
      cb(err);
    } else {
 ...
 */
RQLocalTree.prototype.existsExt = function (context, name, cb) {
  if (this.isDownloading(context, name)) {
    // downloading files will not be reported as existing
    cb(null, false);
  } else {
    this.source.existsExt(context, name, cb);
  }
};
 ...
 */
RQLocalTree.prototype.openExt = function (context, name, cb) {
  var self = this;
  var logger = context.spi();
  if (self.isDownloading(context, name)) {
 ...
 */
RQLocalTree.prototype.listExt = function (context, pattern, cb) {
  var self = this;
  self.source.listExt(context, pattern, function (err, files) {
    if (err) {
      cb(err);
    } else {
      var result = [];
      async.each(files, function (file, eachCb) {
        if (file.getName() != consts.WORK_DIR && !self.isDownloading(context, file.getPath())) {
 ...
 */
RQLocalTree.prototype.createFileExt = function (context, name, cb) {
  var self = this;
  var logger = context.spi();
  ...
  if (self.isDownloading(context, name)) {
    ...
    if (self.cacheInfoOnly) {
      self.source.openExt(context, name, _handleFile);
    } else {
      self.source.createFileExt(context, name, _handleFile);
    }
  }
};
 ...
 */
RQLocalTree.prototype.createDirectoryExt = function (context, name, cb) {
  this.source.createDirectoryExt(context, name, cb);
};
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQLocalTree.prototype.deleteExt = function (context, name, cb) {
  var self = this;
  var logger = context.spi();
  ...
  function doDelete(deleteCb) {
    if (self.cacheInfoOnly) {
      ...
      self.source.deleteExt(context, name, function (err) {
        if (err) {
          cb(err);
        } else {
          deleteCb();
    ...
    doDelete(function () {
      self.cacheInfoExists(context, name, function (err, exists) {
        if (err) {
          ...
          self.source.deleteExt(context, self.getInfoFilePath(name), cb);
        } else {
          cb();
        }
      });
...
RQLocalTree.prototype.deleteDirectoryExt = function (context, name, cb) {
  ...
  self.listExt(context, Path.join(name, '*'), function (err, items) {
    if (err) {
      cb(err);
      ...
      self.source.deleteDirectoryExt(context, name, cb);
    } else {
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQLocalTree.prototype.renameLocal = function (context, oldName, newName, newRemote, cb) {
  var logger = context.spi();
  ...
  function _deleteCacheInfo(deleteName, delCb) {
    self.cacheInfoExists(context, deleteName, function (err, exists) {
      if (err) {
        cb(err);
      } else if (exists) {
        logger.debug('%s rename involved cache info file already exists, removing', deleteName);
        self.source.deleteExt(context, self.getInfoFilePath(deleteName), function (err) {
          if (err) {
            cb(err);
          } else {
            delCb();
  ...
  function _doRename(renameCb) {
    ...
    if (self.cacheInfoOnly) {
      ...
      self.source.renameExt(context, oldName, newName, function (err) {
        if (err) {
          cb(err);
        } else {
          renameCb();
  ...
  if (self.isDownloading(context, oldName)) {
    ...
    _doRename(function () {
      _deleteCacheInfo(oldName, function () {
        _deleteCacheInfo(newName, function () {
          ...
          self.source.openExt(context, newName, function (err, renamed) {
            if (err) {
              cb(err);
            } else {
              // if the target work file already existed, don't flag the file as newly created.
              _createWorkFile.call(self, context, renamed, newCreated, !newCreated, newRemote, function (createErr) {
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQLocalTree.prototype.renameExt = function (context, oldName, newName, cb) {
  this.renameLocal(context, oldName, newName, false, cb);
};
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQLocalTree.prototype.refreshExt = function (context, folderPath, deep, cb) {
  this.source.refreshExt(context, folderPath, deep, cb);
};
