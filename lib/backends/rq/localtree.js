...
RQLocalTree.prototype.waitOnDownload = function (context, path, cb) {
  var logger = context.spi();
  var self = this;
  self.isDownloading(context, path, function (err, isDownloading) {
    if (err) {
      cb(err);
 ...
 */
RQLocalTree.prototype.setDownloading = function (context, path, isDownloading, cb) {
  var logger = context.spi();
  var self = this;
  var downloadingFile = _getDownloadingPath.call(this, path);
  self.isDownloading(context, path, function (err, downloading) {
    if (downloading == isDownloading) {
      // nothing to be done
      cb();
    } else if (isDownloading) {
      logger.debug('creating downloading file %s', downloadingFile);
      self.source.createFileExt(context, downloadingFile, function (err) {
        cb(err);
      });
    } else {
      logger.debug('removing downloading file %s', downloadingFile);
      self.source.deleteExt(context, downloadingFile, function (err) {
        cb(err);
        _notifyDownloadComplete.call(self, context, path);
      });
    }
  });
};
...
RQLocalTree.prototype.download = function (context, remote, name, cb) {
  ...
  function _resetDownloading(callback) {
    self.setDownloading(context, name, false, function (dlErr) {
      if (dlErr) {
  ...
  self.setDownloading(context, name, true, function (err) {
    if (err) {
      cb(err);
    } else {
