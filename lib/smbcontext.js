...
function _formatArguments(toFormat) {
  ...
  toFormat[0] = '[' + this.label + '][' + this.requestId + '] ' + message;
  return toFormat;
}

function WrappedLog(wrapped, label, requestId) {
...
/**
 * Constructs a new context instance using the provided options.
 * @param {Object} options Options to use to create the new context.
 * @constructor
 */
function SMBContext(options) {
...
function _getLog(name) {
  if (!this.logs[name]) {
    this.logs[name] = new WrappedLog(loggers.get(name), this.getLabel(), this.requestId);
  }
