 ...
 */
var DAMFile = function (filePath, content, fileLength, tree, context) {
  if (!(this instanceof DAMFile)) {
    return new DAMFile(filePath, content, fileLength, tree, context);
  }

  JCRFile.call(this, filePath, content, fileLength, tree, context);
};
 ...
 */
DAMFile.createInstance = function (context, filePath, tree, content, fileLength, cb) {
  ...
  async.seq(getContent, getFileLength)(function (err, metaData, length) {
    if (err) {
      ...
      cb(null, new DAMFile(filePath, metaData, length, tree, context));
    }
  });
};
...
DAMFile.prototype.delete = function (cb) {
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      // succeeded
      // invalidate cache
      self.tree.share.invalidateContentCache(this.context, self.filePath, self.isDirectory());
      self.dirty = false;
      if (self.localFile) {
