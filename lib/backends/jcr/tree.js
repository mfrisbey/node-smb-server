 ...
 */
JCRTree.prototype.createFileInstance = function (context, filePath, content, fileLength, cb) {
  ...
  if (_isDotFile(filePath)) {
    ...
    JCRFile.createInstance(context, filePath, this, content, fileLength, cb);
  }
};
 ...
 */
JCRTree.prototype.createFileInstancesFromContent = function (context, content, path, namePattern, cb) {
  var logger = context.spi();
  var results = {};
  var uniqueResults = {};
  if (!content) {
  ...
  async.each(_.keys(entries),
    function (p, callback) {
      self.createFileInstance(context, p, entries[p], function (err, f) {
        if (err) {
          callback(err);
        } else {
          if (f.getName()) {
...
JCRTree.prototype.fetchFileLength = function (path, cb) {
  ...
  var options = this.share.applyRequestDefaults(this.context, {
    url: url,
    method: 'HEAD'
  });
...
JCRTree.prototype.existsExt = function (context, name, cb) {
  ...
  var options = this.share.applyRequestDefaults(context, {
    url: url,
    method: 'HEAD',
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      // failed
      context.spi().error('failed to determine existence of %s', name, err);
 ...
 */
JCRTree.prototype.openExt = function (context, name, cb) {
  var logger = context.spi();
  logger.debug('[%s] tree.open %s', this.share.config.backend, name);
  if (this.tempFilesTree && this.isTempFileName(name)) {
    this.tempFilesTree.openExt(context, name, cb);
    return;
  }
  this.createFileInstance(context, name, cb);
};
 ...
 */
JCRTree.prototype.listExt = function (context, pattern, cb) {
  var logger = context.spi();
  ...
  function listTempFiles(next) {
    if (self.tempFilesTree) {
      self.tempFilesTree.listExt(context, pattern, function (err, tmpFiles) {
        if (err) {
  ...
  function listRemoteFiles(tmpFiles, next) {
    ...
    function getContent(done) {
      self.share.getContent(context, parentPath, true, done);
    }

    function createEntries(content, done) {
      self.createFileInstancesFromContent(context, content, parentPath, fileName, function (err, files) {
        if (err) {
          done(err);
        } else {
  ...
  if (fileName === '*') {
    ...
    if (this.isTempFileName(pattern)) {
      if (this.tempFilesTree) {
        this.tempFilesTree.listExt(context, pattern, cb);
      } else {
        cb(null, []);
      }
      ...
      this.createFileInstance(context, pattern, function (err, file) {
        cb(err, [ file ]);
      })
    }
  }
 ...
 */
JCRTree.prototype.createFileExt = function (context, name, cb) {
  var logger = context.spi();
  ...
  if (this.tempFilesTree && this.isTempFileName(name)) {
    ...
    this.tempFilesTree.createFileExt(context, name, cb);
    return;
  }
  ...
  emptyStream.pipe(
    webutils.submitRequest(options, function (err, resp, body) {
      if (err) {
        ...
        self.createFileInstance(context, name, null, 0, cb);
      }
    })
  );
};
 ...
 */
JCRTree.prototype.createDirectoryExt = function (context, name, cb) {
  var logger = context.spi();
  logger.debug('[%s] tree.createDirectory %s', this.share.config.backend, name);
  if (this.tempFilesTree && this.isTempFileName(name)) {
    // make sure parent path exists
    mkdirp.sync(Path.join(this.tempFilesTree.share.path, utils.getParentPath(name)));
    this.tempFilesTree.createDirectoryExt(context, name, cb);
    return;
  }
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
JCRTree.prototype.deleteExt = function (context, name, cb) {
  var logger = context.spi();
  logger.debug('[%s] tree.delete %s', this.share.config.backend, name);
  if (this.tempFilesTree && this.isTempFileName(name)) {
    this.tempFilesTree.deleteExt(context, name, cb);
    return;
  }
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      // succeeded
      // invalidate cache
      self.share.invalidateContentCache(context, name, false);
      cb();
    }
  });
};
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
JCRTree.prototype.deleteDirectoryExt = function (context, name, cb) {
  var logger = context.spi();
  logger.debug('[%s] tree.deleteDirectory %s', this.share.config.backend, name);
  if (this.tempFilesTree && this.isTempFileName(name)) {
    this.tempFilesTree.deleteDirectoryExt(context, name, cb);
    return;
  }
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      // succeeded
      // invalidate cache
      self.share.invalidateContentCache(context, name, true);
      if (self.tempFilesTree) {
        // now cleanup tmp files shadow directory
        self.tempFilesTree.deleteDirectoryExt(context, name, function (ignored) {
          cb();
        });
      } else {
        cb();
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
JCRTree.prototype.renameExt = function (context, oldName, newName, cb) {
  var logger = context.spi();
  ...
  if (this.tempFilesTree && this.isTempFileName(oldName) && this.isTempFileName(newName)) {
    this.tempFilesTree.renameExt(context, oldName, newName, cb);
    return;
  }
  ...
  if (this.isTempFileName(oldName) || this.isTempFileName(newName)) {
    ...
    srcTree.openExt(context, oldName, function (err, srcFile) {
      if (err) {
        cb(err);
        return;
      }
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
JCRTree.prototype.refreshExt = function (context, folderPath, deep, cb) {
  var logger = context.spi();
  logger.debug('[%s] tree.refresh %s, %d', this.share.config.backend, folderPath, deep);

  // invalidate cache
  this.share.invalidateContentCache(context, utils.getParentPath(folderPath), deep);
