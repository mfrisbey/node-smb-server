...
RQProcessor.prototype.sync = function (config, cb) {
  ...
  self.currCb = cb;
  var context = self.tree.createContext();
  var logger = context.spi();
          ...
          if (err) {
            ...
            self.rq.completeRequest(context, item.path, item.name, function (err) {
              if (err) {
                ...
                removeActiveUpload();
                self.tree.share.invalidateContentCache(context, self.unicodeNormalize(item.path), true);
                if (method != 'DELETE') {
                  self.tree.refreshWorkFiles(context, self.unicodeNormalize(refPath), function (err) {
                    if (err) {
...
RQProcessor.prototype.start = function (config) {
  var context = this.tree.createContext().withLabel("RQProcessor.Start");
  var logger = context.spi();
...
RQProcessor.prototype.stop = function () {
  var context = this.tree.createContext().withLabel('RQProcessor.Stop');
  var logger = context.spi();
