...
function handle(context, msg, commandId, commandParams, commandData, commandParamsOffset, commandDataOffset, connection, server, descCb, cb) {
  ...
  descCb(msg.oldFileName + ' > ' + msg.newFileName, function () {
    ...
    tree.existsExt(context, msg.oldFileName, function (err, exists) {
      if (err) {
        cb({
      ...
      tree.renameExt(context, msg.oldFileName, msg.newFileName, function (err) {
        cb({
