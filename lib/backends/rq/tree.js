...
function _deleteFile(tree, toDelete, force, cb) {
  ...
  toDelete.canDelete(function (err, canDelete) {
    if (err) {
      ...
      var filePath = toDelete.getPath();
      tree.queueDataExists(filePath, function (err, exists) {
        if (err) {
          //cb(err);
        } else {
          if (!exists) {
              logger.debug('cannot delete file %s, emitting conflict event', filePath);
              tree.emitSyncConflict(filePath);
          }
        }
        cb(null, false);
      });
    ...
    if(findConflicts) {
      ...
      file.canDelete(function (err, canDelete) {
        if(!err && !canDelete) {
          ...
          self.queueDataExists(filePath, function (err, exists) {
          if (err) {
            //cb(err);
          } else {
            if (!exists) {
              logger.debug('A conflict has been found for file %', filePath);
