 ...
 */
var FSFile = function (filePath, stats, tree, context) {
  var logger = context.spi();
  logger.debug('[fs] file.open %s', filePath);
  if (!(this instanceof FSFile)) {
    return new FSFile(filePath, stats, tree, context);
  }
  this.stats = stats;
 ...
 */
FSFile.createInstance = function (context, filePath, tree, cb) {
  ...
  fs.stat(realPath, function (err, stats) {
    if (err) {
      ...
      cb(null, new FSFile(filePath, stats, tree, context));
    }
  });
};
