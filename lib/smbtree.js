...
SMBTree.prototype.createContext = function () {
  return this.spiTree.createContext();
};
...
SMBTree.prototype.exists = function (name, cb) {
  this.existsExt(this.createContext().withLabel('SMBTree.Exists').requestless(), name, cb);
};

/**
 * Extended version of exists method that tests whether or not the specified file exists.
 ...
 * @param {Boolean} cb.exists true if the file exists; false otherwise
 */
SMBTree.prototype.existsExt = function (context, name, cb) {
  this.spiTree.existsExt(context, utils.normalizeSMBFileName(name), cb);
};
...
SMBTree.prototype.openOrCreate = function (name, createDisposition, openTargetDirectory, cb) {
  this.openOrCreateExt(this.createContext().withLabel('SMBTree.OpenOrCreate').requestless(), name, createDisposition, openTargetDirectory, cb);
};

/**
 * Extended version of openOrCreate method to open or create an existing file/directory.
 *
 * @param {SMBContext} context Additional information that can provide enhanced capabilities throughout the workflow.
 * @param {String} name file name
 * @param {Number} createDisposition flag specifying action if file does/does not exist
 ...
 * @param {SMBFile} cb.file opened file
 */
SMBTree.prototype.openOrCreateExt = function (context, name, createDisposition, openTargetDirectory, cb) {
  var self = this;
  ...
  function open(callback) {
    self.spiTree.openExt(context, name, function (err, file) {
      if (err) {
        callback(err);
        return;
      }
      ...
      var result = new SMBFile(file, self, openAction, fid, context);
  ...
  if (createDisposition === common.FILE_OPEN || createDisposition === common.FILE_OVERWRITE) {
    ...
    self.existsExt(context, name, function (err, exists) {
      if (err) {
        cb(err);
        return;
      }
...
SMBTree.prototype.open = function (name, cb) {
  this.openExt(this.createContext().withLabel('SMBTree.Open').requestless(), name, cb);
};

/**
 * Extended version of open method to open an existing file/directory.
 *
 * @param {SMBContext} context Additional information that can provide enhanced capabilities throughout the workflow.
 * @param {String} name file name
 * @param {Function} cb callback called with the opened file
 ...
 * @param {SMBFile} cb.file opened file
 */
SMBTree.prototype.openExt = function (context, name, cb) {
  var self = this;
  this.spiTree.openExt(context, utils.normalizeSMBFileName(name), function (err, file) {
    if (err) {
      cb(err);
    } else {
      var fid = ++SMBTree.fidCounter;
      var result = new SMBFile(file, self, common.FILE_OPENED, fid, context);
      self.files[fid] = result;
      cb(null, result);
    }
  });
...
SMBTree.prototype.reopen = function (name, fid, cb) {
  this.reopenExt(this.createContext().withLabel('SMBTree.Reopen').requestless(), name, fid, cb);
};

/**
 * Extended version of repoen to reopen an existing file/directory using an already assigned fid.
 * Special purpose method called when an already open SMBFile instance
 * is renamed in order to make sure that the internal state of the
 * wrapped File instance is consistent with the new path/name.
 *
 ...
 * @param {SMBFile} cb.file reopened file
 */
SMBTree.prototype.reopenExt = function (context, name, fid, cb) {
  var self = this;
  this.spiTree.openExt(context, utils.normalizeSMBFileName(name), function (err, file) {
    if (err) {
      cb(err);
...
SMBTree.prototype.list = function (pattern, cb) {
  this.listExt(this.createContext().withLabel('SMBTree.List').requestless(), pattern, cb);
};
 ...
 * @param {SMBFile[]} cb.files array of matching files
 */
SMBTree.prototype.listExt = function (context, pattern, cb) {
  var npattern = utils.normalizeSMBFileName(pattern);
  var self = this;
  this.spiTree.listExt(context, npattern, function (err, files) {
    if (err) {
      cb(err);
    } else {
      var results = files.map(function (file) {
        return new SMBFile(file, self, undefined, undefined, context);
      });
      cb(null, results);
...
SMBTree.prototype.createFile = function (name, cb) {
  this.createFileExt(this.createContext().withLabel('SMBTree.CreateFile').requestless(), name, cb);
};
 ...
 * @param {SMBFile} cb.file created file
 */
SMBTree.prototype.createFileExt = function (context, name, cb) {
  var self = this;
  var nname = utils.normalizeSMBFileName(name);

  this.spiTree.createFileExt(context, nname, function (err, file) {
    if (err) {
      cb(err);
      return;
    }
    var fid = ++SMBTree.fidCounter;
    var result = new SMBFile(file, self, common.FILE_CREATED, fid, context);
    self.files[fid] = result;
    cb(null, result);
...
SMBTree.prototype.createDirectory = function (name, cb) {
  this.createDirectoryExt(this.createContext().withLabel('SMBTree.CreateDirectory').requestless(), name, cb);
};
 ...
 * @param {SMBFile} cb.file created directory
 */
SMBTree.prototype.createDirectoryExt = function (context, name, cb) {
  var self = this;
  var nname = utils.normalizeSMBFileName(name);

  this.spiTree.createDirectoryExt(context, nname, function (err, file) {
    if (err) {
      cb(err);
      return;
    }
...
SMBTree.prototype.delete = function (name, cb) {
  this.deleteExt(this.createContext().withLabel('SMBTree.Delete').requestless(), name, cb);
};
 ...
 * @param {Boolean} cb.deleted true if the file could be deleted; false otherwise
 */
SMBTree.prototype.deleteExt = function (context, name, cb) {
  var self = this;
  var nname = utils.normalizeSMBFileName(name);

  this.spiTree.deleteExt(context, nname, function (err) {
    cb(err);
    if (!err) {
...
SMBTree.prototype.deleteDirectory = function (name, cb) {
  this.deleteDirectoryExt(this.createContext().withLabel('SMBTree.DeleteDirectory').requestless(), name, cb);
};
 ...
 * @param {Boolean} cb.deleted true if the directory could be deleted; false otherwise
 */
SMBTree.prototype.deleteDirectoryExt = function (context, name, cb) {
  var self = this;
  var nname = utils.normalizeSMBFileName(name);

  this.spiTree.deleteDirectoryExt(context, nname, function (err) {
    cb(err);
    if (!err) {
      self.notifyChangeListeners(common.FILE_ACTION_REMOVED, nname);
...
SMBTree.prototype.rename = function (nameOrFile, newName, cb) {
  this.renameExt(this.createContext().withLabel('SMBTree.Rename').requestless(), nameOrFile, newName, cb);
};
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
SMBTree.prototype.renameExt = function (context, nameOrFile, newName, cb) {
  var self = this;
  ...
  this.spiTree.renameExt(context, nOldName, nNewName, function (err) {
    if (err) {
      cb(err);
      return;
    }
    if (targetFID) {
      self.reopenExt(context, nNewName, targetFID, cb);
    } else {
      cb();
    }
...
SMBTree.prototype.refresh = function (folderPath, deep, cb) {
  this.refreshExt(this.createContext().withLabel('SMBTree.Refresh').requestless(), folderPath, deep, cb);
};
 ...
 * @param {String|Error} cb.error error (non-null if an error occurred)
 */
SMBTree.prototype.refreshExt = function (context, folderPath, deep, cb) {
  var self = this;
  // give SPI impl a chance to invalidate cache
  this.spiTree.refreshExt(context, folderPath, deep, function (err) {
    if (err) {
      cb(err);
    } else {
...
SMBTree.prototype.clearCache = function (cb) {
  var self = this;
  if (self.spiTree) {
    self.spiTree.clearCacheExt(self.spiTree.createContext().withLabel('SMBTree.ClearCache'), cb);
  } else {
