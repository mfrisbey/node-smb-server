...
var RQFile = function (openFile, tree, context) {
  ...
  File.call(this, openFile.getPath(), tree, context);
};
 ...
 */
RQFile.createInstance = function (context, openFile, tree, cb) {
  cb(null, new RQFile(openFile, tree, context));
};
...
RQFile.prototype.cacheFile = function (cb) {
  var rqlog = this.context.rq();
  var logger = this.context.spi();
  ...
  if (self.syncDone) {
    ...
    self.local.waitOnDownload(this.context, self.filePath, function (err) {
      if (err) {
        self.tree.handleErr(self.context, cb, err);
      } else {
        // TODO use async.js to simplify code
        // first, determine if a local cache of the file already exists
        self.local.existsExt(self.context, self.filePath, function (err, exists) {
          if (err) {
            self.tree.handleErr(self.context, cb, err);
          } else if (exists) {
            ...
            self.local.openExt(self.context, self.filePath, function (err, localFile) {
              if (err) {
                self.tree.handleErr(self.context, cb, err);
              } else {
                        ...
                        if (err) {
                          ...
                          localFile.canDelete(function (err, canDelete) {
                            if (err) {
                              ...
                              self.tree.queueDataExists(self.context, self.getPath(), function (err, exists) {
                                if (err) {
                                  self.tree.handleErr(self.context, cb, err);
                                } else {
                                  if (!exists) {
                                  ...
                                  if (err) {
                                    ...
                                    remoteFile.close(function (err) {
                                      if (err) {
                                        ...
                                        self.local.deleteExt(self.context, self.filePath, function (err) {
                                          if (err) {
                                            self.tree.handleErr(self.context, cb, err);
                                          } else {
                                            self.syncDone = false;
            ...
            self.local.download(self.context, self.remote, self.getPath(), function (err, cachedFile) {
              if (err) {
                self.tree.handleErr(self.context, cb, err);
              } else {
  ...
  this.openFile.close(function (err) {
    if (err) {
      ...
      if (self.dirty) {
        ...
        self.tree.isCreatedLocally(context, self.getPath(), function (err, isLocal) {
          if (err) {
            self.tree.handleErr(self.context, cb, err);
          } else if (isLocal) {
            logger.debug('%s is newly created, queuing creation', self.getPath());
            self.tree.queueData(context, self.getPath(), 'PUT', false, cb);
          } else {
            logger.debug('%s is being updated, queuing update', self.getPath());
            self.tree.queueData(context, self.getPath(), 'POST', false, cb);
          }
        });
      } else {
