...
var EmptyFile = require('./emptyfile');
  ...
  self.local.exists(name, function (err, localExists) {
    if (err) {
      ...
      if (localExists) {
        ...
        // on OSX, there are several files that the OS attempts to open regardless of
        // whether they exist or not. These include .hidden and .TemporaryItems. If the
        // files are reported as not found when opened then the SMB connection will drop,
        // so for these types of files just treat them as "empty" in-memory files
        logger.debug('[rq] attempting to open path that does not exist %s', name);
        cb(null, new EmptyFile(name, name == '/.TemporaryItems', self));
      }
    }
  });
};
