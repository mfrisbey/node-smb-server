...
function SMBShare(smbServer, spiShare) {
  ...
  this.shid = ++SMBShare.shidCounter;

  this.files = {};
  this.listeners = {};
}

SMBShare.shidCounter = 0;
...
SMBShare.prototype.connect = function (session, shareLevelPassword, cb) {
  var self = this;
  this.spiShare.connect(session, shareLevelPassword, cb);
};

/**
 * Disconnects the share by calling its underlying SPI share.
 * @param {Function} cb Invoked when the operation is complete.
 * @param {SMBError} cb.error non-null if an error occurred.
 */
SMBShare.prototype.disconnect = function () {
  var self = this;
  ...
  // delegate to spi
  this.spiShare.disconnect(function (err) {
    if (err) {
...
/**
 * Creates a new instance of an SMBTree and returns it.
 * @param {SMBServer} server The server creating the tree.
 * @param {SMBContext} context The context that will be given to the tree.
 * @returns {SMBTree} The newly created tree instance.
 */
SMBShare.prototype.createTree = function (server) {
  return new SMBTree(server, this, this.spiShare.createTree(this.spiShare.createContext()));
};

SMBShare.prototype.getFile = function (fid) {
  return this.files[fid];
};

SMBShare.prototype.setFile = function (fid, file) {
  this.files[fid] = file;
};

SMBShare.prototype.closeFile = function (fid, cb) {
...
SMBShare.prototype.flush = function (cb) {
 ...
 */
SMBShare.prototype.registerChangeListener = function (mid, file, deep, completionFilter, cb) {
  var self = this;
  var listener = {
    mid: mid,
    path: file.getPath(),
 ...
 */
SMBShare.prototype.notifyChangeListeners = function (action, name, newName) {
 ...
 */
SMBShare.prototype.cancelChangeListener = function (mid) {
