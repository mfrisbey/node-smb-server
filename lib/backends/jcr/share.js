...
JCRShare.prototype.getContent = function (context, path, deep, cb) {
  ...
  this.fetchContent(context, path, deep, function (err, content) {
    if (err) {
      cb(err);
      return;
    }
    if (content) {
      ...
      self.invalidateContentCache(context, path, deep);
    }
    cb(null, content);
  });
};

JCRShare.prototype.invalidateContentCache = function (context, path, deep) {
...
JCRShare.prototype.fetchContent = function (context, path, deep, cb) {
  if (path === Path.sep) {
    path = '';
  }
  var depth = deep ? 2 : 1;
  var url = this.buildContentUrl(path, depth);
  var opts = this.applyRequestDefaults(context, null, url);
 ...
 */
JCRShare.prototype.getLocalFile = function (context, path, lastModified, cb) {
  var logger = context.spi();
  var self = this;
  ...
  function cacheResource(path, callback) {
    ...
    self.fetchResource(context, path, function (err, localFilePath) {
      if (err) {
        cb(err);
        return;
      }
...
JCRShare.prototype.createTreeInstance = function (content, tempFilesTree, context) {
  return new JCRTree(this, content, tempFilesTree, context);
};

JCRShare.prototype.applyRequestDefaults = function (context, opts, url) {
  var def = {context: context};
  if (url) {
    def.url = url;
  }
  if (this.auth) {
...
JCRShare.prototype.connect = function (session, shareLevelPassword, cb) {
  ...
  function getContent(done) {
    self.getContent(self.createContext().withLabel('JCRShare.Connect'), Path.sep, false, done);
  }
  ...
  function connectTempShare(content, tempDir, done) {
    ...
    tmpShare.connect(session, null, function (error) {
      done(error, content, tmpShare);
    });
  }

  async.waterfall([ getContent, createTempDir, prepopulateTempDir, connectTempShare ], function (err, content, share) {
    if (err) {
...
JCRShare.prototype.createTree = function (context) {
  var tmpTree = this.tmpShare ? this.tmpShare.createTree(context) : null;
  return this.createTreeInstance(this.content, tmpTree, context);
};
