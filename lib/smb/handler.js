...
function _handleRequest(msg, connection, server, cb) {
  ...
  async.eachSeries(msg.commands,
    function (cmd, callback) {
      var context;
      if (msg.header) {
        if (msg.header.tid !== undefined) {
          var tree = server.getTree(msg.header.tid);
          if (tree) {
            context = tree.createContext().withRequestId(msg.header.mid);
          }
        }
      }
      if (!context) {
        context = new SMBContext().withLabel('SMB.HANDLER');
      }
      ...
      if (handler) {
        // process command
        executeHandler(command, handler, context, msg, cmd.commandId, cmd.params, cmd.data, cmd.paramsOffset, cmd.dataOffset, connection, server, function (result) {
          if (!result) {
