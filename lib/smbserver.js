...
function SMBServer(config, authenticator) {
  ...
  _.forEach(config.shares,
    function (shareCfg, name) {
      ...
      self.shareids[share.shid] = share;
 ...
 * @param {SMBSession} cb.session authenticated session
 */
SMBServer.prototype.connectShare = function (session, shareName, shareLevelPassword, cb) {
  ...
  var self = this;
  share.connect(session, shareLevelPassword, function (err) {
    if (err) {
      cb(err);
    } else {
      cb(null, share);
...
SMBServer.prototype.getTree = function (tid) {
  var share = this.shareids[tid];
  if (share) {
    return share.createTree(this);
  } else {
    return null;
  }
};

SMBServer.prototype.disconnectShare = function (tid) {
  var share = this.shareids[tid];
  if (share) {
    var shareName = share.getName();
    share.disconnect();
    // emit event
