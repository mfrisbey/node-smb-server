 ...
 */
var JCRTree = function (share, content, tempFilesTree, context) {
  if (!(this instanceof JCRTree)) {
    return new JCRTree(share, content, tempFilesTree, context);
  }
...
JCRTree.prototype.applyRequestDefaults = function (options, url) {
  return this.share.applyRequestDefaults(this.getContext(), options, url);
};

JCRTree.prototype.invalidateContentCache = function (path, deep) {
  this.share.invalidateContentCache(this.getContext(), path, deep);
};

JCRTree.prototype.getContent = function (path, deep, cb) {
  this.share.getContent(this.getContext(), path, deep, cb);
};

JCRFile.prototype.getLocalFile = function (path, lastModified, cb) {
  this.share.getLocalFile(this.getContext(), path, lastModified, cb);
};
...
JCRTree.prototype.fetchFileLength = function (path, cb) {
  ...
  var options = this.applyRequestDefaults({
    url: url,
    method: 'HEAD'
  });
...
JCRTree.prototype.list = function (pattern, cb) {
  ...
  function listRemoteFiles(tmpFiles, next) {
    ...
    function getContent(done) {
      self.getContent(parentPath, true, done);
    }
...
JCRTree.prototype.delete = function (name, cb) {
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      // succeeded
      // invalidate cache
      self.invalidateContentCache(name, false);
      cb();
    }
  });
};
...
JCRTree.prototype.deleteDirectory = function (name, cb) {
  ...
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      // succeeded
      // invalidate cache
      self.invalidateContentCache(name, true);
      if (self.tempFilesTree) {
...
JCRTree.prototype.refresh = function (folderPath, deep, cb) {
  ...
  // invalidate cache
  this.invalidateContentCache(utils.getParentPath(folderPath), deep);
