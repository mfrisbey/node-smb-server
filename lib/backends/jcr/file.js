 ...
 */
var JCRFile = function (filePath, content, fileLength, tree, context) {
  var logger = context.spi();
  logger.debug('[%s] file.open %s', tree.share.config.backend, filePath);
  if (!(this instanceof JCRFile)) {
    return new JCRFile(filePath, content, fileLength, tree, context);
  }
  ...
  File.call(this, filePath, tree, context);
};
 ...
 */
JCRFile.createInstance = function (context, filePath, tree, content, fileLength, cb) {
  var logger = context.spi();
  ...
  function getContent(callback) {
    if (content) {
      ...
      tree.share.getContent(context, filePath, false, function (err, content) {
        if (content) {
...
JCRFile.prototype.ensureGotLocalCopy = function (cb) {
  ...
  var self = this;
  this.tree.share.getLocalFile(this.context, this.filePath, this.lastModified(), function (err, localFilePath) {
    if (err) {
      cb(err);
    } else {
...
JCRFile.prototype.syncLocalChanges = function (cb) {
  ...
  var options = this.tree.share.applyRequestDefaults(self.context, {
    url: url,
    method: 'PUT',
    headers: {
        ...
        if (err) {
          ...
          fs.fstat(localFile.fd, function (err, stats) {
            if (err) {
              ...
              self.tree.share.invalidateContentCache(self.context, self.filePath, false);
  ...
  this.ensureGotLocalCopy(function (err, localFile) {
    ...
    fs.ftruncate(localFile.fd, length, function (err) {
      if (err) {
        ...
        self.tree.share.getContent(self.context, self.filePath, false, function (err, content) {
          if (content) {
...
JCRFile.prototype.delete = function (cb) {
  ...
  var url = this.tree.share.buildResourceUrl(this.filePath);
  var options = this.tree.share.applyRequestDefaults(this.context, {
    url: url,
    method: 'DELETE'
  });
  var self = this;
  webutils.submitRequest(options, function (err, resp, body) {
    if (err) {
      ...
      // succeeded
      // invalidate cache
      self.tree.share.invalidateContentCache(self.context, self.filePath, self.isDirectory());
      self.dirty = false;
      if (self.localFile) {
