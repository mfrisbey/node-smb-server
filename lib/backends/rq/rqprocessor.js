...
function RQProcessor(share, options) {
  ...
  this.rq.on('itemupdated', function (data) {
    ...
    var logger = data.context.spi();
    logger.debug('processor received itemupdated event for %s', itemPath);
    if (self.abortUpload(data.context, itemPath)) {
    ...
    for (var property in self.activeRequests) {
      if (property.length > prefix.length) {
        if (property.substr(0, prefix.length) == prefix) {
          if (self.abortUpload(data.context, property)) {
            self.sync(self.currConfig, self.currCb);
          }
        }
      }
 ...
 */
RQProcessor.prototype.abortUpload = function (context, itemPath) {
...
RQProcessor.prototype.sync = function (config, cb) {
  ...
  self.currCb = cb;
  var context = self.share.createContext().withLabel('RQProcessor.Sync');
  var tree = self.share.createTree(context);
  var logger = context.spi();
  var processItemMethod = function (item, method, processCb) {
    ...
    if (url.match(/\/\./g)) {
      ...
      var options = self.share.applyRequestDefaults(context, {
        url: url,
        method: method,
        headers: {}
      });
          ...
          if (err) {
            ...
            self.rq.completeRequest(context, item.path, item.name, function (err) {
              if (err) {
                ...
                removeActiveUpload();
                self.share.invalidateContentCache(context, self.unicodeNormalize(item.path), true);
                if (method != 'DELETE') {
...
RQProcessor.prototype.start = function (config) {
  var context = this.share.createContext().withLabel("RQProcessor.Start");
  var logger = context.spi();
  ...
  var doSync = function (cb) {
    ...
    self.sync(config, function (err) {
      ...
      self.rq.purgeFailedRequests(context, config.maxRetries, function (err, purged) {
        if (err) {
          self.emit('error', err);
        } else {
          if (purged.length) {
...
RQProcessor.prototype.stop = function () {
  var context = this.share.createContext().withLabel('RQProcessor.Stop');
  var logger = context.spi();
  ...
  for (var property in self.activeRequests) {
    self.abortUpload(context, property);
  }
  this.stopped = true;
};
