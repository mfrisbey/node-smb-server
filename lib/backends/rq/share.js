...
RQShare.prototype.onServerEvent = function (context, eventName, data) {
  ...
  if (eventName == 'downloadasset') {
    var path = data.path;
    logger.info('downloading file %s to local cache by request of server', path);
    this.remote.fetchResource(context, this.remoteEncodePath(path), function (err, localPath) {
      if (err) {
        logger.error('unable to download file %s to local cache due to error', path, err);
      } else {
        logger.info('successfully downloaded file %s to local cache at %s by request of server', path, localPath);
      }
    });
  }
};
