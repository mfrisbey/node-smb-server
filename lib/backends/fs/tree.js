 ...
 */
FSTree.prototype.createFileInstance = function (context, name, cb) {
  FSFile.createInstance(context, name, this, cb);
};
 ...
 */
FSTree.prototype.existsExt = function (context, name, cb) {
  context.spi().debug('[%s] tree.exists %s', this.share.config.backend, name);
  context.perf().debug('%s Tree.exists.fs.stat', name);
  var self = this;
  fs.stat(Path.join(this.share.path, name), function (err, stats) {
    if (err && err.code !== 'ENOENT') {
      ...
      context.spi().debug('[%s] tree.exists %s > %s', self.share.config.backend, name, !err);
      cb(null, !err);
    }
  });
};
 ...
 */
FSTree.prototype.openExt = function (context, name, cb) {
 ...
 */
FSTree.prototype.listExt = function (context, pattern, cb) {
  var logger = context.spi();
  var perflog = context.perf();
  ...
  function createFileInstances(names, next) {
    async.reduce(names, [],
      function (memo, name, callback) {
        self.createFileInstance(context, Path.join(parentPath, name), function (err, file) {
          if (err) {
 ...
 */
FSTree.prototype.createFileExt = function (context, name, cb) {
  var logger = context.spi();
  var perflog = context.perf();
        ...
        if (err) {
          ...
          fs.close(fd, function (err) {
            if (err) {
              ...
              self.createFileInstance(context, name, cb);
            }
          });
        }
      });
 ...
 */
FSTree.prototype.createDirectoryExt = function (context, name, cb) {
  var logger = context.spi();
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
FSTree.prototype.deleteExt = function (context, name, cb) {
  var logger = context.spi();
  var perflog = context.perf();
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
FSTree.prototype.deleteDirectoryExt = function (context, name, cb) {
  var logger = context.spi();
  var perflog = context.perf();
 ...
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
FSTree.prototype.renameExt = function (context, oldName, newName, cb) {
  var logger = context.spi();
  var perflog = context.perf();
